 ...
 */
{
  ...
  // +++++++++++++++++++++++ Added
  for (int j=0; j<500; j+=100){printf("image[%d] = %d\n", j, *(unsigned short int*) (image+j) );}

  // Writing file
  int num=fwrite( image, 2, xsize*ysize, image_file);  // ptr to image; byte of single element; total number of bytes; name image

  // (image, 1, xsize*ysize*color_depth, image_file); original
  printf("num = %d", num);
 ...
 */
{
  ...
  *xsize = *ysize = *maxval = 0;
  ...
  unsigned short int* c_vec=malloc(*xsize * *ysize * sizeof(short int));
  if (k > 0)
    {

      //fread(&c,1,(*xsize * *ysize),image_file);for (int ii=0; ii<100; ++ii)  printf("Here it is: %d\n", *(&c+ii) );//{printf("line[%d] = %hd\n", ss,*(unsigned short int*) (line+ss));}
        printf("End ************************* End");
	k = sscanf(line, "%d%*c%d%*c%d%*c", xsize, ysize, maxval); // correct line but how is it parsed?
        printf("%d, %d, %d", *xsize, *ysize, *maxval);
	if ( k < 3 )
	fscanf(image_file, "%d%*c", maxval);
 // fread(&c_vec,2,(*xsize * *ysize),image_file);for (int ii=0; ii<1000; ii+=10)  printf("Here it is: %d\n", *(c_vec+ii) );
    }
  else
    {
  ...
  // ++++++++++++++++++ Added 
  int color_depth = 1 + ( *maxval > 255 );
  unsigned int size = *xsize * *ysize * color_depth; //number of bytes in total
  ...
  if ( maxval > 255 )
    {
      ...
      for ( int i = 0; i < size; i+= 1 )  // modified i+=2 to i+=1
